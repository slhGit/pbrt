...
namespace pbrt {
	...
	Film::Film(const Point2i &resolution, const Bounds2f &cropWindow, std::unique_ptr<Filter> filt, Float diagonal, const std::string &filename, 
		Float scale, Float maxSampleLuminance)
		: fullResolution(resolution), diagonal(diagonal * .001), filter(std::move(filt)), filename(filename), scale(scale), maxSampleLuminance(maxSampleLuminance) 
	{
		...
		LOG(INFO) << "Created film with full resolution " << resolution << ". Crop window of " << cropWindow << " -> croppedPixelBounds " << croppedPixelBounds;
	...
	Film::Film(const Film& f, std::unique_ptr<Filter> filt, const std::string &filename)
		: fullResolution(f.fullResolution), diagonal(f.diagonal), filter(std::move(filt)), filename(filename), scale(f.scale), maxSampleLuminance(f.maxSampleLuminance)
	{
		croppedPixelBounds = f.croppedPixelBounds;
