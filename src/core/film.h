...
namespace pbrt {
	...
	class Film {
	  ...
	  public:
		// Film Public Methods
		Film(const Point2i &, const Bounds2f &, std::unique_ptr<Filter>, Float, const std::string &, Float, Float = Infinity);
		Film(const Film&, std::unique_ptr<Filter>, const std::string &);

		Bounds2i GetSampleBounds() const;
		Bounds2f GetPhysicalExtent() const;
		
		std::unique_ptr<FilmTile> GetFilmTile(const Bounds2i &);
		
		void MergeFilmTile(std::unique_ptr<FilmTile>);
		void SetImage(const Spectrum *) const;
		void AddSplat(const Point2f &, Spectrum);
		void WriteImage(Float = 1);
		void Clear();
	...
	class FilmTile {
	  public:
		...
		FilmTile(const Bounds2i &pixelBounds, const Vector2f &filterRadius, const Float *filterTable, int filterTableSize, Float maxSampleLuminance)
			: pixelBounds(pixelBounds), filterRadius(filterRadius), invFilterRadius(1 / filterRadius.x, 1 / filterRadius.y),
			  filterTable(filterTable), filterTableSize(filterTableSize), maxSampleLuminance(maxSampleLuminance) 
		{
	  ...
	  private:
		...
		const Float maxSampleLuminance;
		friend class Film;
		friend class TaggedFilmTile;
		friend class AdaptiveSamplerIntegrator;
		friend class DammertzSamplerIntegrator;
		friend class FilmTileWrap;
		friend class FilmTileWrap2;
